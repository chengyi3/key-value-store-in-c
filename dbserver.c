...
int main(int argc, char **argv)
{
    ...
    /* TODO: your code here */
    pthread_t thread1;
    int e = pthread_create(&thread1, NULL, &listen_thread, NULL);
    pthread_t thread_worker[num_thread];
    for (int i = 0; i < num_thread; i++) {
        pthread_create(&thread_worker[i], NULL, worker, (void *) args);
         pthread_create(&thread_worker[i], NULL, &worker,  &args);
    }
    typeof(&args);
    //
