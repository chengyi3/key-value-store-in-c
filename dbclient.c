...
//has to be void*
void * do_set_th(void *set_args) {
    struct set_args *arg = (struct set_args *)set_args;
    do_set(arg->args, arg->key, arg->val, arg->len, arg->result,arg->quiet);
}

void * do_get_th(void *get_args) {
    struct get_args *arg = (struct get_args *)get_args;
    do_get(arg->args, arg->name, NULL, NULL, NULL);
}

void * do_del_th(void *del_args) {
    struct del_args *arg = (struct del_args *)del_args;
    do_del(arg->args, arg->name, NULL, 0);
}
...
void do_test(struct args *a)
{
    ...
    srandom(5600);
   //can write  a specific key, and get and delete
    ...
    pthread_t thread[num_threads];
        ...
        for (i = 0; i < num_threads; i++) {
            ...
            a->key = malloc(sizeof(char) * 30);
            a->val = malloc(sizeof(char) * 4096);
            gen_randkey(a->key);
            gen_randval(a->val);
            k[i] = a->key;
            v[i] = a->val;
            ...
            arg.key = a->key;
            arg.val = a->val;
            arg.len = strlen(a->val);
            arg.result = NULL;
            arg.quiet = 0;
            (void) arg;
           pthread_create(&thread[i], NULL, do_set_th, &arg);
    ...
    for (i = 0; i < num_threads; i++) {
        ...
        a->key = k[i];
        arg.name = a->key;
        pthread_create(&thread[i], NULL, do_get_th, &arg);
        pthread_mutex_unlock(&p);
    }
    ...
    for (i = 0; i < num_threads; i++) {
        ...
        arg.args = a;
        arg.name = k[i];
        pthread_create(&thread[i], NULL, do_del_th, &arg);
        pthread_mutex_unlock(&p);
    }
